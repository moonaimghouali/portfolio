...
const ProjectViewSm = ({p}) => {
  return (
    <Link href={p.live} target='_blank' className='w-full aspect-video rounded-md overflow-clip relative'>
        <div className='absolute z-30 text-white text-lg font-semibold bottom-3 left-3'>{p.title}</div>
        ...
        <Image src={p.previewImage} width={500} height={500} alt='image project' className='z-10 absolute w-full h-full object-cover'/>
    </Link>
  )
}
